...
import com.mulesoft.connectors.leanixconnectormule4.internal.metadata.GetSynchronizationRunWarningsOutputMetadataResolver;
...
public class GetSynchronizationRunWarningsOperation extends BaseRestOperation {
  private static final String OPERATION_PATH = "/synchronizationRuns/{id}/warnings";

  private static final RestRequestBuilder.QueryParamFormat QUERY_PARAM_FORMAT =
      RestRequestBuilder.QueryParamFormat.MULTIMAP;

  public GetSynchronizationRunWarningsOperation() {}

  /**
   * Returns the warnings of a synchronization run
   *
   * <p>This operation makes an HTTP GET request to the /synchronizationRuns/{id}/warnings endpoint
   *
   * @param config the configuration to use
   * @param connection the connection to use
   * @param idUriParam The ID of the synchronization run
   * @param offsetQueryParam The zero-based index of the first element to retrieve
   * @param limitQueryParam The number of elements that should be retrieved
  ...
  @DisplayName("Get Synchronization Run Warnings")
  @Summary("Returns the warnings of a synchronization run")
  @MediaType("application/json")
  @OutputResolver(output = GetSynchronizationRunWarningsOutputMetadataResolver.class)
  public void getSynchronizationRunWarnings(
      @Config RestConfiguration config,
      @Connection RestConnection connection,
      @DisplayName("id") @Summary("The ID of the synchronization run") String idUriParam,
      @Optional(defaultValue = "0")
          @DisplayName("offset")
          @Summary("The zero-based index of the first element to retrieve")
          Integer offsetQueryParam,
      @Optional(defaultValue = "100")
          @DisplayName("limit")
          @Summary("The number of elements that should be retrieved")
